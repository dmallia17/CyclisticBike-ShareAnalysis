...
`r round(max(all_tripdata_v3$ride_length) / (60 * 60 * 24))` days...
...
```
Out of over 2 million casual rides, only `r total_casual_exceeding_member_max`
...
violating_case_num
```
Thankfully we only have `r violating_case_num` cases, and can discard these
...
## Bike Type Usage
We can examine which types of bikes
```{r}
